...
impl Data for GuiApp {
    ...
    fn render_image_window(&mut self, ctx: &egui::Context) {
            egui::Window::new("Pdf viewer").collapsible(true).resizable(true).default_size(Vec2::new(1000.0, 1000.0)).show(ctx, |ui| {
                egui::ScrollArea::new([true , true]).show(ui, |ui| {
                    ui.add(egui::Image::new(self.texture.as_ref().unwrap().id(), [500.0, 700.0]));
    ...
    fn render_header(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ui.label("Project repo:");
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.add_space(PADDING);
                    if ui.button(RichText::new("Add Company").color(Color32::GRAY)).clicked() {
                        self.create_company = true;
                    }
                    ui.add_space(PADDING);
                    if ui.button(RichText::new("Add Partner").color(Color32::GRAY)).clicked() {
                        self.create_partner = true;
                    }
                    ui.add_space(PADDING);
                    if ui.button(RichText::new("Add Service").color(Color32::GRAY)).clicked() {
                        self.create_service = true;
                    }
                });
                ui.add_space(PADDING);
    ...
    fn render_create_partner(&mut self, ctx: &egui::Context) {
        egui::Window::new("Create Partner").resizable(true).show(ctx, |ui| {
            ui.horizontal(|ui| {
                ui.with_layout(egui::Layout::left_to_right(Align::Center), |ui| {
                    ui.vertical(|ui| {
                        ui.heading("Partner data");
    ...
    fn render_create_service(&mut self, ctx: &egui::Context) {
        egui::Window::new("Create Service").resizable(true).show(ctx, |ui| {
            ui.horizontal(|ui| {
                ui.with_layout(egui::Layout::left_to_right(Align::Center), |ui| {
                    ui.vertical(|ui| {
